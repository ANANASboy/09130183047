...
class VoltageControl(MultiAgentEnv):
    ...
    def get_obs(self):
        ...
        for agent_id in range(self.n_agents - 1):
            ...
            # Local voltage and angle
            vm = self.powergrid.res_bus["vm_pu"][sgen_bus]
            va = self.powergrid.res_bus["va_degree"][sgen_bus] * np.pi / 180 # convert to radians
            ...
            obs_pv = np.array([vm, va, p_pv, last_q])
            obs_list.append(obs_pv)
            
        # Agent 6: ESS (global observation)
        ess_agent_id = self.n_agents - 1
        
        # Global voltage and angle
        v_all = self.powergrid.res_bus["vm_pu"].sort_index().to_numpy(copy=True)
        va_all = self.powergrid.res_bus["va_degree"].sort_index().to_numpy(copy=True) * np.pi / 180 # radians
        
        # ESS specific state
        soc = self.soc
        
        # Last action for ESS
        last_p_ess, last_q_ess = self.last_actions[ess_agent_id]
        
        # Time features
        time_features = self._get_time_features()
        
        obs_ess = np.concatenate([v_all, va_all, [soc, last_p_ess, last_q_ess], time_features])
    ...
    def _take_action(self, actions):
        ...
        """
        # Store actions for use in next observation
        self.last_actions = actions

        # Unpack actions: PVs (first 6 agents) and ESS (last agent)
        pv_q_actions = actions[:(self.n_agents - 1), 1]
        ess_p_cmd, ess_q_cmd = actions[self.n_agents - 1, 0], actions[self.n_agents - 1, 1]
        ...
        if self.soc <= 0 and ess_p_cmd > 0:
            ess_p_cmd = 0
        # Prevent charging when full
        if self.soc >= 1 and ess_p_cmd < 0:
            ess_p_cmd = 0

        # Update SoC based on the (potentially clipped) action
        # Note: discharging (p>0) decreases soc
        self.soc = self.soc - ess_p_cmd * dt / self.ess_capacity
        ...
        # Our agent action ess_p_cmd > 0 means discharging.
        self.powergrid.storage.at[0, 'p_mw'] = -ess_p_cmd
        self.powergrid.storage.at[0, 'q_mvar'] = ess_q_cmd
    ...
    def _calc_reward(self, info={}):
        ...
        """
        # --- 1. Standard Info Dict Population (from old implementation) ---
        ...
        # Voltage Reward (Primary Goal)
        # Dead-band approach: penalize deviations outside [0.95, 1.05]
        v_dev = np.abs(v - v_ref) - (self.v_upper - v_ref) # v_ref=1.0, v_upper=1.05
        r_voltage = -100.0 * np.sum(np.maximum(0, v_dev))

        # ESS SoC Centering Reward
        r_soc_center = -10.0 * abs(soc - 0.5)

        # ESS SoC Limit Penalty (heavy penalty for going outside [0.1, 0.9])
        # abs(soc - 0.5) > 0.4 means soc is outside [0.1, 0.9]
        r_soc_limit = -500.0 * max(0, abs(soc - 0.5) - 0.4)

        # ESS Action Cost (penalize large power flow)
        r_ess_action = -0.5 * abs(ess_p)
        
        # --- 4. Combine Rewards ---
        reward = r_voltage + r_soc_center + r_soc_limit + r_ess_action
